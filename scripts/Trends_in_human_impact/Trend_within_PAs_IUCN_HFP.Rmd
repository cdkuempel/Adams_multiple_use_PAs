...
```{r}
chi <- list.files(here("output_data/hfp/projected/"), full=TRUE,
                  pattern="Proj")
...
```{r}
trend <- raster(here("data/trend/All_terrestrial_hfp_slope.tif"))
```
...
for(i in 1:length(iucn)){
  ...
  writeRaster(wdpa_ras, here(paste0("outout_data/WDPA/iucn/",iucn[i],"_hfp_raster.tif")), overwrite = T)
...
chi_data_df <- data.frame(chi_data) %>%
  ...
  mutate(year = as.numeric(substring(pressure, 10, 13))) %>%
  mutate(pressure = "chi")
...
all<-all %>% 
  mutate(pressure = "hfp",
         year = ifelse(year == 9, 2009,
                       ifelse(year == 10, 2010,
                              ifelse(year == 11, 2011,
                                     ifelse(year == 12, 2012,
                                            ifelse(year == 13, 2013, NA))))))

all_trend<-all_trend %>% 
  mutate(pressure = "hfp_trend")

write.csv(all, here("output_data/results/IUCN_hfp.csv"))
write.csv(all_trend, here("output_data/results/IUCN_hfp_trend.csv"))
```
...
for(i in 2:length(strict)){
  ...
  writeRaster(wdpa_ras, here(paste0("output_data/WDPA/iucn/",strict[i],"_hfp_raster.tif")), overwrite = T)
...
chi_data_df <- data.frame(chi_data) %>%
  ...
  mutate(year = as.numeric(substring(pressure, 9, 12))) %>%
  mutate(pressure = "hfp")
...
```{r}
write.csv(all, here("output_data/results/Strict_cat_IUCN_hfp.csv"))
write.csv(all_trend, here("outout_data/results/Strict_cat_IUCN_hfp_trend.csv"))
```
...
```{r}
all_wdpa_ras<-raster(here("output_data/WDPA/WDPA_All_terrestrial_raster.tif"))
wdpa_strict<-raster(here("output_data/WDPA/iucn/Strict_hfp_raster.tif"))
wdpa_not_strict<-raster(here("output_data/WDPA/iucn/Not_strict_hfp_raster.tif"))
wdpa_other<-raster(here("output_data/WDPA/iucn/Other_hfp_raster.tif"))
```
...
write.csv(all_chi_eez_df, here("output_data/results/Country_hfp_results.csv"))

st_write(all_chi_eez, here("output_data/results/Country_hfp_results.shp"), delete_layer = TRUE)
```
...
write.csv(all_chi_trend_eez_df, here("output_data/results/Country_hfp_trend_results.csv"))

st_write(all_chi_trend_eez, here("output_data/results/Country_hfp_trend_results.shp"), delete_layer = TRUE)
```
